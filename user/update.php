...
if (strlen($_SESSION['cmsaid']==0)) {
  header('location: ../includes/logout.php');
      ...
      $msg="";
      if(isset($_POST["save"])){
        $ret1=mysqli_query($con,"UPDATE `user` SET `fname`='".$_POST["fname"]."',`lname`='".$_POST["lname"]."',`phone`=".(int)$_POST["phone"].",`address`='".$_POST["addr"]."',`city`='".$_POST["city"]."',`state`='".$_POST["state"]."',`country`='".$_POST["country"]."',`pin`='".$_POST["pin"]."' Where `id`=".$_SESSION['cmsaid']);
        if($ret1){
...
<table style="background-color: var(--grey-alt);">
  ...
  <?php
  $ret=mysqli_query($con,"SELECT * FROM user  WHERE id=".$_SESSION['cmsaid']);
