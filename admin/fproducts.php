...
if (strlen($_SESSION['cmsaid']==0)) {
    header('location:../includes/logout.php');
        ...
        if(isset($_GET['pid']))
        {
          $query= mysqli_query($con,"DELETE FROM fproduct WHERE id=".(int)$_GET["pid"]);
          if(!$query){
   ...
   <div class="title-container">
    ...
    <table style="background-color: var(--grey-alt);">
        ...
        while ($row=mysqli_fetch_array($ret)) {
            $ret1=mysqli_query($con,"SELECT email FROM farmer WHERE id=".$row["farmerid"]);
        ...
        ?>
            ...
            <td data-label="Action"> <?php echo '<a class="btn-danger btn" href=" ./fproducts.php?pid='.$row["id"].'&email='.$row1["email"].'">Remove</a>';?></td>
            </tr>
            <?php
        }}
        ?>
