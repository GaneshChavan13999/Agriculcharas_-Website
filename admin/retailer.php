...
if (strlen($_SESSION['cmsaid']==0)) {
    header('location:../includes/logout.php');
        ...
        if(isset($_GET['pid']))
        {
          $query= mysqli_query($con,"DELETE FROM retailer WHERE id=".(int)$_GET["pid"]);
          if(!$query){
              $msg=" Product is not removed. Please try again later.";
          }
          else{
                            include "../includes/mail.php";
                            $m="Hello <br> Your Retailer Account has been deleted by Admin Please contact Admin for further details. <br> Thank you. ";
   ...
   <div class="title-container">
        <div class="section-titles">
          <div class="section-title active" data-id="latest">
            <span class="dot"></span>
            <h1 class="primary-title">All Retailers</h1>
          </div>
        </div>
    </div>
    <table style="background-color: var(--grey-alt);">
        ...
        <tbody><?php
        $ret=mysqli_query($con,"SELECT * FROM retailer");
        ...
        ?>
            ...
            <td data-label="Action"> <?php echo '<a class="btn-danger btn" href=" ./retailer.php?pid='.$row["id"].'&email='.$row["email"].'">Remove</a>';?></td>
            </tr>
            <?php
        }}
        ?>
