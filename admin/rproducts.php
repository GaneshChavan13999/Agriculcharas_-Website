...
if (strlen($_SESSION['cmsaid']==0)) {
    header('location:../includes/logout.php');
        ...
        if(isset($_GET['pid']))
        {
          $query= mysqli_query($con,"DELETE FROM rproduct WHERE id=".(int)$_GET["pid"]);
          if(!$query){
              $msg=" Product is not removed. Please try again later.";
          }
          else{
                            include "../includes/mail.php";
                            $m="Hello <br> Your product has been deleted by Admin Please contact Admin for further details. <br> Thank you. ";
                            $s=mailto($_GET["email"],"Product removed",$m);
          }
   ...
   <div class="title-container">
        <div class="section-titles">
          <div class="section-title active" data-id="latest">
            <span class="dot"></span>
            <h1 class="primary-title">All Retailer's Products</h1>
          </div>
        </div>
    </div>
    <table style="background-color: var(--grey-alt);">
        ...
        <tbody><?php
        $ret=mysqli_query($con,"SELECT * FROM rproduct");
        $num=mysqli_num_rows($ret);
        if($num >0){
            $count=0;
        while ($row=mysqli_fetch_array($ret)) {
            $ret1=mysqli_query($con,"SELECT email FROM retailer WHERE id=".$row["rid"]);
            $row1=mysqli_fetch_array($ret1);
            $count++;
        ?>
            <tr>
            <td scope="row" data-label="SR. NO."><?php echo $count;?></td>
            <td data-label="By"><?php echo $row1["email"];?></td>
            ...
            <td data-label="Action"> <?php echo '<a class="btn-danger btn" href=" ./rproducts.php?pid='.$row["id"].'&email='.$row1["email"].'">Remove</a>';?></td>
            </tr>
            <?php
        }}
        ?>
